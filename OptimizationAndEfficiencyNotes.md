...
I don't like having `unsafeBitcast` in the wild, if I can avoid it, so I put that in a extension on `FixedWidthInteger`, where I can also assert that the other bits are 0.  So now we can do this:
...
Now our outer `if` only has a single opportunity to mispredict.  We can't do anything about the inner `if`, but again it will only be mispredicted once.  That's about all we can do to help the branch predictor.  But those multiplications in the loop stand out.  They are just integer multiplication, so they're not especially slow, but it turns out we can make them faster.  In `q̂ * vNextToLast`, only changes in response to decrementing `q̂` by 1.  Decrementing `q̂` has the effect of subtracting `vNextToLast` from the product, so let's do that instead.
...
The only other thing I do is give the conditions names, but while I think that optimizes human comprehension, that's not the sort of optimization this document intends to cover.  The actual routine has the named conditions. 
